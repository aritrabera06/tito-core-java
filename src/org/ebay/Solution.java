...
public class Solution {
	...
	private static String reformatString(String str) {
		...
		separateCharAndNums(str, SIZE, numSB, alphaSB);

		if ((numSB.length() + 1) == alphaSB.length()) {
			for (int indx = 0; indx < alphaSB.length(); indx++) {
				if (indx == alphaSB.length() - 1) {
					patternSB.append(alphaSB.charAt(indx));
				} else {
					patternSB.append(alphaSB.charAt(indx));
					patternSB.append(numSB.charAt(indx));
				}
			}
		} else if (numSB.length() == alphaSB.length()) {
			for (int indx = 0; indx < alphaSB.length(); indx++) {
				patternSB.append(alphaSB.charAt(indx));
				patternSB.append(numSB.charAt(indx));
			}
			...
			for (int indx = 0; indx < alphaSB.length(); indx++)
				patternSB.append(alphaSB.charAt(indx));
			for (int indx = 0; indx < numSB.length(); indx++)
				patternSB.append(numSB.charAt(indx));
		}
	...
	private static void separateCharAndNums(String str, int SIZE, StringBuffer numSB, StringBuffer alphaSB) {
		for (int indx = 0; indx < SIZE; indx++) {
			if (Character.isDigit(str.charAt(indx)))
				numSB.append(str.charAt(indx));
			if (Character.isAlphabetic(str.charAt(indx)))
				alphaSB.append(str.charAt(indx));
		}
		System.out.println("alphaSB : " + alphaSB.toString());
		System.out.println("numSB   : " + numSB.toString());
	}
}
